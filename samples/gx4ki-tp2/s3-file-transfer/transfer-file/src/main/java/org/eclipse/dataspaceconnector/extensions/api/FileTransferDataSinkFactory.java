...
class FileTransferDataSinkFactory implements DataSinkFactory {
    ...
    public boolean canHandle(DataFlowRequest request) {
        monitor.info("FileTransferDataSinkFactory.canHandle");
        return "file".equalsIgnoreCase(request.getDestinationDataAddress().getType());
    }

    @Override
    public @NotNull Result<Boolean> validate(DataFlowRequest request) {
        monitor.info("FileTransferDataSinkFactory.validate");
        return Result.success(true);
    }

    @Override
    public DataSink createSink(DataFlowRequest request) {
        monitor.info("FileTransferDataSinkFactory.createSink");
