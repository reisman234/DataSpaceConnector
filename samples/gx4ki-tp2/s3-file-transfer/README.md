...
negotiationId=$(curl -s -X POST -H "Content-Type: application/json" -H "X-Api-Key: password" -d @samples/04.0-file-transfer/contractoffer.json "http://localhost:9192/api/v1/data/contractnegotiations"); echo "NegotiationID: $negotiationId"


agreementId=$(curl -s -X GET -H 'X-Api-Key: password' "http://localhost:9192/api/v1/data/contractnegotiations/${negotiationId}" | jq -r .contractAgreementId); echo "AgreementId: $agreementId"

curl -X POST "http://localhost:9191/api/file/test-document?connectorAddress=http://localhost:8282/api/v1/ids/data&destination=/tmp/testfile&contractId=${agreementId}"
```
